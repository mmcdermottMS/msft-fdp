...
def main(ingestion: List[func.EventHubEvent], context) -> None:
    ...
    with tracer.start_as_current_span("ingest_items", context=extract(carrier)):
        logging.info(f"FDP - Ingest: Ingested {len(ingestion)} events.")
        items: List[Item] = []
    ...
    for partition_key, items in items_by_order_id.items():        
        ...
        for item in items:
            try:
                ...
                logging.info(F"FDP - Ingest: Published {len(event_data_batch)} items in a batch")
                ...
                try:
                    ...
                    logging.error("FDP - Ingest: Message too large to fit into EventDataBatch object")

        producer.send_batch(event_data_batch)
        logging.info(F"FDP - Ingest: Published {len(event_data_batch)} items to partition key {partition_key}")